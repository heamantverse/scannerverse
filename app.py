...
# 🤖 🛠️ ટેલિગ્રામ કનેક્શન સેટઅપ (તમારા બંને આંકડા અહીં લખો)
...
def send_telegram_alert(message_text):
    if (
        TELEGRAM_TOKEN
        and TELEGRAM_TOKEN != "8879164929:AAHo9RfH2hBpSW062hP0J1aMbx9xMdAJ90g"
    ):
        try:
...
suggestions_pool = [
    ...
    "DEEPAKNTR",
    "EXIDEIND",
    "GLENMARK", "GODREJPROP", "GRANULES", "GUJGASLTD", "INDIGO", "IRCTC", "JSWENERGY", "JUBLFOOD",
...
def analyze_index_daily(ticker_name, display_name):
    try:
        ...
        return {
            ...
            "Current Spot": f"₹ {current_price:,.2f}",
            "Nearby Node Level": f"⚡ {closest_node}",
        }
    except:
        return None
...
def analyze_stock_yearly(ticker_name):
    try:
        ...
        return {
            ...
            "Calculated Levels": filtered_levels,
        }
    except:
        return None
...
if not st.session_state["authenticated"]:
    ...
    st.markdown(
        "<h1 style='text-align: center;'>🦅 Proprietary Structural Matrix Scanner</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align: center; color: #8892b0;'>Premium Quant Infrastructure Tool</p>",
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    with st.spinner("તમામ ઇન્ડાઇસિસ મેટ્રિક્સ લોડ થઈ રહ્યો છે..."):
        index_results = [
            analyze_index_daily(ticker, name)
            for name, ticker in all_market_indices.items()
            if analyze_index_daily(ticker, name) is not None
        ]
